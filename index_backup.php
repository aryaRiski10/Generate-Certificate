...
$offset = 120; // Ubah nilai ini untuk menaikkan teks lebih tinggi atau lebih rendah
...
// Kalimat sebelum dan sesudah judul
$before = 'For successfully completing the "';
$after = '" program.';

// Hitung posisi awal (manual)
$combined = $before . $judul_kelas . $after;

// Total lebar kalimat
$box = imagettfbbox($fontsize_class, $angle, $font, $combined);
$total_width = $box[2] - $box[0];
$start_x = ($image_width - $total_width) / 2;
$y_text = $y + 120;

// Gambar bagian awal
imagettftext($sertifikat, $fontsize_class, $angle, $start_x, $y_text, $black, $font, $before);

// Ukur lebar teks "before"
$before_box = imagettfbbox($fontsize_class, $angle, $font, $before);
$before_width = $before_box[2] - $before_box[0];

// Gambar judul (dengan bold)
imagettftext($sertifikat, $fontsize_class, $angle, $start_x + $before_width, $y_text, $black, $font_bold, $judul_kelas);

// Ukur lebar judul
$judul_box = imagettfbbox($fontsize_class, $angle, $font_bold, $judul_kelas);
$judul_width = $judul_box[2] - $judul_box[0];

// Gambar bagian akhir
imagettftext($sertifikat, $fontsize_class, $angle, $start_x + $before_width + $judul_width, $y_text, $black, $font, $after);
...
<body>
  ...
  <img src="<?php echo $image_url; ?>" alt="Sertifikat" style="max-width:40%; border:1px solid #ccc;"><br><br>
